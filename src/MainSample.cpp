...
/**
 * The entrance function of sample generation given ground truth scap files.
 */
int main(int argc, char *argv[]) {
  ...
  app.add_flag("-u,--uncut", has_cut_file,
               "The records of cut points between.");
  ...
  generate_stroke_positive_samples(input, pos_stroke_samples);
  generate_stroke_negative_samples(input, neg_stroke_samples, &matching_pair);
  generate_cluster_positive_samples(input, pos_cluster_samples);
  ...
  std::cout << scap_filename << " sample time: " << std::fixed
            << std::setprecision(6)
  ...
  // 3. Write
  write_samples(output_csv_filename + "_pos_stroke.csv", pos_stroke_samples);
  write_samples(output_csv_filename + "_neg_stroke.csv", neg_stroke_samples);
  write_samples(output_csv_filename + "_pos_cluster.csv", pos_cluster_samples);
  write_samples(output_csv_filename + "_neg_cluster.csv", neg_cluster_samples);

  write_filtered_samples(output_csv_filename + "_filtered.csv",
                         filtered_samples);
