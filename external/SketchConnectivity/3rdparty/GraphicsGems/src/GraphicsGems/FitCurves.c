...
static void(*DrawBezierCurve)(int, BezierCurve, int, int, double*);
...
{
    assert(nPts < MAXPOINTS);
    DrawBezierCurve = callback;
...
void FitSingleCubic(Point2* d, int nPts, double error, void(*callback)(int, Point2*, int, int, double*))
{
    ...
    int first = 0;
    int last = nPts - 1;
    Vector2 tHat1 = ComputeLeftTangent(d, 0);
    Vector2 tHat2 = ComputeRightTangent(d, nPts - 1);
    ...
    if (nPts == 2) {
        ...
        DrawBezierCurve(3, bezCurve, first, last, NULL);
        free((void *)bezCurve);
        return;
    }

    double* u = ChordLengthParameterize(d, first, last);
    BezierCurve bezCurve = GenerateBezier(d, first, last, u, tHat1, tHat2);
    for (int i = 0; i < maxIterations; i++) {
        double* uPrime = Reparameterize(d, first, last, u, bezCurve);
        free((void *)bezCurve);
        bezCurve = GenerateBezier(d, first, last, uPrime, tHat1, tHat2);
	int splitPoint;
        double maxError = ComputeMaxError(d, first, last,
                   bezCurve, uPrime, &splitPoint);
        if (maxError < error) {
            DrawBezierCurve(3, bezCurve, first, last, uPrime);
            free((void *)u);
            free((void *)bezCurve);
            free((void *)uPrime);
            return;
    ...
    DrawBezierCurve(3, bezCurve, first, last, u);
    free((void *)u);
    free((void *)bezCurve);
}
