...
enum SketchState {
  ...
  SS_ADJUST_HEMLINE_WRINKLE_EDGES // adjusting wrinkle edge directions by
                                  // draging the end node
...
protected: // data
  ...
  Polyline2D curSketchPolyline; // the curretnly drawing sketch curve, in NDCS
                                // [-1,1]*[-1,1]
...
public: // attribute
  ...
  size_t getCurSketchPolylineSize(void) const {
    return curSketchPolyline.points.size();
  }
  ...
  size_t getSketchedPolylineSize(void) const {
    return sketchedPolylines.size();
  }
...
public: // operations
  ...
  // if p is not too close (squared_dist > POLYLINESAMPLETHRES) to the
  ...
  void samplePolylinePoint(const Point2D &p, std::int64_t timestamp) {
    ...
    std::cout << "sampling: " << p.x << ", " << p.y << " - "
              << (p - curSketchPolyline.points.back().first).SqLength()
              << std::endl;
    if ((p - curSketchPolyline.points.back().first).SqLength() >=
        POLYLINESAMPLETHRES) {
  ...
  void endSampling(void) {
    ...
    std::cout << "Sampled " << sketchedPolylines.back().points.size()
              << " points for polyline " << (sketchedPolylines.size() - 1)
              << "." << std::endl;
  }
  ...
  void deleteLastPolyline(void) {
    if (sketchedPolylines.size() > 0) {
      ...
      printf("Last polyline deleted, %zu polylines remained.\n",
  ...
  bool pickEndNode(size_t polylineI, const Point2D &p) {
    ...
    if ((sketchedPolylines[polylineI].points.back().first - p).SqLength() <=
        PICKNODETHRES) {
  ...
  std::string to_string() const {
    ...
    for (auto const &sketch : sketchedPolylines) {
      if (sketch.points.size() == 1) {
        std::cout << "Warning: Skipping sketch with only one point."
                  << std::endl;
        continue;
      }
  ...
  void from_string(std::string sketch_str) {
    ...
    if (sketch_str.find_first_of("@") != std::string::npos) {
      std::string thickness_str =
        sketch_str.substr(sketch_str.find_first_of("@") + 1);
      std::vector<std::string> strs_digit =
        SketchUI::split_str(thickness_str, "\n");
    ...
    for (size_t i = 0; i < strs.size(); i++) {
      ...
      Polyline2D poly;
      poly.from_string(strs[i].substr(strs[i].find_first_of("{")), i);
      if (poly.width < 0)
  ...
  void from_string4svg(std::string sketch_str) {
    std::string pattern(">");
    std::vector<std::string> strs = split_str(sketch_str, pattern);
    int ind = 0;
    for (size_t i = 0; i < strs.size(); i++) {
      if (strs[i].find("path") != std::string::npos) {
        std::string scd_pattern = "\"";
        std::vector<std::string> features = split_str(strs[i], scd_pattern);
        Polyline2D poly;
        poly.from_string4svg(strs[i], ind);
        for (size_t j = 0; j < features.size() / 2; j++) {
          if (features[2 * j].find("stroke-width=") != std::string::npos) {
            thickness = std::stod(features[2 * j + 1]);
          }
        }
        sketchedPolylines.emplace_back(poly);
        ind++;
  ...
  std::string to_svg_string(size_t width, size_t height) const {
    ...
    svg_str += "<svg baseProfile=\"full\" height=\"" + std::to_string(height) +
               "\" version=\"1.1\" viewBox=\"0,0," + std::to_string(width) +
