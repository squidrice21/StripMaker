...
Fitting::Fitting(Context &context)
  : context(context) {}

void Fitting::fit_svg(std::ostream &os, const Input &input,
                      const std::map<int, FittedCurve> &fits, double width,
                      double height) {
...
std::map<int, FittedCurve> Fitting::fit(Input *input) {
  return map_clusters<FittedCurve>(*input,
                                   [&](Cluster &c) { return fit_cluster(c); });
}
...
FittedCurve Fitting::fit_cluster(Cluster cluster) {
  ...
  // 1. Get xsec samples more densely sampled near endpoints
  {
    ...
    cluster.xsecs = parameterization.xsecs_from_params(
  ...
  // 	curve.centerline = fit_positions4one_stroke(cluster);
  // }else{
...
std::vector<double> Fitting::fit_widths(const std::vector<Sample> &samples,
                                        bool periodic) {
...
Fitting::fit_positions(const std::vector<Sample> &samples,
                       const std::vector<glm::dvec2> &tangents, bool periodic) {
  std::vector<glm::dvec2> positions;
  // std::cout << "pos finish 1" << std::endl;
  GRBModel model(context.grb);
  // std::cout << "pos finish 2" << std::endl;
  struct VecVar {
    GRBVar x;
    GRBVar y;
  };
  ...
  for (int i = 0; i < samples.size(); ++i) {
    ...
    if (periodic || i < samples.size() - 1) {
      ...
      double target_len = std::max(
        MIN_DIST, glm::dot(samples[j].point - samples[i].point, tangents[i]));
      tan_terms.emplace_back((vars[j].x - vars[i].x) / target_len -
                             tangents[i].x);
      tan_terms.emplace_back((vars[j].y - vars[i].y) / target_len -
                             tangents[i].y);
    }
  }

  model.setObjective(l2_norm_sq(&model, tan_terms) +
                     POS_WEIGHT * l2_norm_sq(&model, pos_terms));
  context.optimize_model(&model);
  positions.reserve(vars.size());
  for (auto &vec : vars) {
    try {
      positions.emplace_back(vec.x.get(GRB_DoubleAttr_X),
                             vec.y.get(GRB_DoubleAttr_X));
  ...
  // std::cout << "pos finish 4.5" << std::endl;
  if (periodic) {
    positions.push_back(positions.front());
  }
  // std::cout << "pos finish 5" << std::endl;
  return positions;
}

std::vector<glm::dvec2> Fitting::fit_positions4one_stroke(Cluster &cluster) {
  std::vector<glm::dvec2> positions;
  positions.reserve(cluster.strokes.size());
...
std::vector<glm::dvec2>
Fitting::fit_tangents(const std::vector<Sample> &samples, bool periodic) {
  ...
  for (int i = 0; i < samples.size(); ++i) {
    ...
    if (periodic || i < samples.size() - 1) {
      ...
      k_terms.emplace_back(scale *
                           (vars[j].x - vars[i].x - samples[i].k * ortho.x));
      k_terms.emplace_back(scale *
                           (vars[j].y - vars[i].y - samples[i].k * ortho.y));
    }
  }

  model.setObjective(l2_norm_sq(&model, tan_terms) +
                     K_WEIGHT * l2_norm_sq(&model, k_terms));
  context.optimize_model(&model);

  tangents.reserve(vars.size());
  for (auto &vec : vars) {
    tangents.push_back(glm::normalize(
      glm::dvec2{vec.x.get(GRB_DoubleAttr_X), vec.y.get(GRB_DoubleAttr_X)}));
  }

  return tangents;
}

std::vector<Fitting::Sample> Fitting::samples_from_xsec(const Cluster &cluster,
                                                        size_t xsec_idx) {
