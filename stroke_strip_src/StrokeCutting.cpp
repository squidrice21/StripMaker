...
std::map<size_t, size_t> preprocess_cluster(int cut_opt, int width, int height,
                                            Capture *inout_capture,
                                            bool to_reindex) {
  ...
  capture =
    cut.cut_cornucopia(capture, cutindex_to_index, c_record, cut_opt != 0);
  ...
  for (size_t i = 0; i < capture.sketchedPolylines.size(); i++) {
    double step_length =
      capture.getSketchedPolyline(i).totalLen() / min_num_sample_per_stroke;
  ...
  double long_len =
    std::min(5., 0.015 * std::sqrt(width * width + height * height));
  for (size_t i = 0; i < capture.sketchedPolylines.size(); i++) {
    if (capture.getSketchedPolyline(i).totalLen() >= long_len) {
      filtered_capture.sketchedPolylines.emplace_back(
        capture.getSketchedPolyline(i));
    }
  }
  ...
  if (to_reindex) {
    size_t re_ind = 0;
    for (size_t i = 0; i < capture.sketchedPolylines.size(); i++) {
      reindex_to_index.emplace(re_ind,
                               capture.getSketchedPolyline(i).stroke_ind);
      index_to_reindex.emplace(capture.getSketchedPolyline(i).stroke_ind,
                               re_ind);
      cut.update_cut_record(-1 * (int)capture.getSketchedPolyline(i).stroke_ind,
                            re_ind);
...
Capture StrokeCut::cut_sharp_turns(Capture const &capture) const {
  ...
  double sharp_threshold =
    glm::length(glm::dvec2(std::cos(sharp_angle), std::sin(sharp_angle)) -
                glm::dvec2(1, 0));
  ...
  auto cut_stroke = [&stroke_ind, &max_id,
                     &sharp_angle](Sketch const &s) -> std::vector<Sketch> {
    ...
    cut_pos.resize(s.points.size(), false);
    {
      for (size_t i = 1; i + 1 < large_sum_turning.size(); i++) {
        double change_from =
          std::abs(small_sum_turning[i]) - std::abs(small_sum_turning[i - 1]);
        double change_to =
          std::abs(small_sum_turning[i + 1]) - std::abs(small_sum_turning[i]);
...
Capture
StrokeCut::cut_spirals(Capture const &capture,
                       std::map<size_t, size_t> &cutindex_to_index,
                       std::map<size_t, std::vector<size_t>> &c_record) const {
  int max_id = 0;
  for (auto const &s : capture.sketchedPolylines) {
    max_id = std::max(max_id, s.stroke_ind);
  }
  ...
  for (size_t i = 0; i < capture.sketchedPolylines.size(); i++) {
    ...
    for (auto const &s : cut_strokes) {
      c_record[capture.sketchedPolylines[i].stroke_ind].emplace_back(
        s.stroke_ind);
      cut_capture.sketchedPolylines.emplace_back(s);

      if (cut_strokes.size() > 1)
        cutindex_to_index[s.stroke_ind] =
...
std::vector<Sketch> StrokeCut::cut_evenly(std::vector<Sketch> const &strokes,
...
void RDP(std::vector<glm::dvec2> const &points, std::vector<size_t> &indices,
         double epsilon) {
  // Find the point furtherest away from the line connecting the first and last
  // points
  size_t far_index = 0;
  ...
  double max_x_distance = -1;
  {
    ...
    glm::dvec2 normed =
      (glm::length(simp_vector) > std::numeric_limits<double>::epsilon())
        ? glm::normalize(simp_vector)
        : glm::dvec2(1, 0);
    ...
    for (size_t i = 0; i < indices.size(); i++) {
      ...
      double dist =
        glm::length(dist_vector - glm::dot(dist_vector, normed) * normed);
  ...
  if (max_distance > epsilon || folding && max_distance > 5) {
    ...
    std::copy(indices.begin(), indices.begin() + far_index + 1,
              sub_left_indices.begin());
    std::copy(indices.begin() + far_index, indices.end(),
              sub_right_indices.begin());
    ...
    std::copy(sub_left_indices.begin(), sub_left_indices.end(),
              indices.begin());
    std::copy(sub_right_indices.begin() + 1, sub_right_indices.end(),
              indices.begin() + sub_left_indices.size());
  } else { // Or terminate
...
Capture StrokeCut::cut_RDP(Capture const &capture) const {
  ...
  auto cut_stroke = [&max_id,
                     &RDP_epsilon](Sketch const &s) -> std::vector<Sketch> {
    // Simplify using RDP
    std::vector<glm::dvec2> points;
    std::vector<size_t> indices;
    for (size_t i = 0; i < s.points.size(); i++) {
...
Capture StrokeCut::cut_RDP_sharp_turns(Capture const &capture) const {
  ...
  auto compute_cut_positions_RDP = [&RDP_epsilon](Sketch const &s,
                                                  std::vector<bool> &cut_pos) {
    // Simplify using RDP
    std::vector<glm::dvec2> points;
    std::vector<size_t> indices;
  ...
  auto compute_cut_positions_sharp_turn =
    [&sharp_angle](Sketch const &s, std::vector<bool> &cut_pos) {
      std::vector<double> local_turning;
      local_turning.emplace_back(0);
      for (size_t i = 1; i + 1 < s.points.size(); i++) {
        size_t prev = i - 1;
  ...
  auto cut_stroke =
    [&max_id](Sketch const &s,
              std::vector<bool> const &cut_pos) -> std::vector<Sketch> {
    std::vector<Sketch> cut;
    Sketch cur_s;
    cur_s.stroke_ind = s.stroke_ind;
  ...
  for (size_t i = 0; i < capture.sketchedPolylines.size(); i++) {
    ...
    for (size_t j = 0; j < capture.sketchedPolylines[i].points.size(); j++) {
      points.emplace_back(
        to_glm_vec(capture.sketchedPolylines[i].points[j].first));
    ...
    for (size_t ind : indices) {
      sharp_sketch.points.emplace_back(
        capture.sketchedPolylines[i].points[ind]);
    }
    ...
    std::vector<Sketch> cut_strokes =
      cut_stroke(capture.sketchedPolylines[i], cut_pos);
    for (auto const &s : cut_strokes) {
      cut_capture.sketchedPolylines.emplace_back(s);
    }
  }
...
Capture StrokeCut::cut_hooks_simple(Capture const &capture) const {
  ...
  auto cut_stroke = [&max_id](Sketch const &s) -> std::vector<Sketch> {
    ...
    cut_pos.resize(s.points.size(), false);
    if (s.points.size() > 2) {
      auto is_a_turn = [](SketchPoint p1, SketchPoint p2,
                          SketchPoint p3) -> bool {
        return glm::dot((to_glm_vec(p1) - to_glm_vec(p2)),
                        (to_glm_vec(p2) - to_glm_vec(p3))) < 0;
      };
      cut_pos[1] =
        is_a_turn(s.points[0].first, s.points[1].first, s.points[2].first);
      cut_pos[cut_pos.size() - 2] =
        is_a_turn(s.points[s.points.size() - 1].first,
                  s.points[s.points.size() - 2].first,
                  s.points[s.points.size() - 3].first);

      for (size_t i = 1; i + 1 < s.points.size(); i++) {
        cut_pos[i] = is_a_turn(s.points[i - 1].first, s.points[i].first,
                               s.points[i + 1].first);
      }
    }
...
void smooth_stroke(Sketch &s) {
  ...
  auto get_weighted_laplacian = [](glm::dvec2 p1, glm::dvec2 p2,
                                   glm::dvec2 p3) -> glm::dvec2 {
  ...
  auto handle_end = [&get_weighted_laplacian](Sketch &s, size_t end_ind,
                                              int to_neighbor) {
    int n_ind = end_ind + to_neighbor;
    assert(n_ind >= 0 && n_ind < s.points.size());

    glm::dvec2 n_lap = get_weighted_laplacian(
      to_glm_vec(s.points[n_ind + to_neighbor].first),
      to_glm_vec(s.points[n_ind].first), to_glm_vec(s.points[end_ind].first));
    glm::dvec2 end_lap = get_weighted_laplacian(
      to_glm_vec(s.points[n_ind].first), to_glm_vec(s.points[end_ind].first),
    ...
    glm::dvec2 tan = to_glm_vec(s.points[end_ind - to_neighbor].first) -
                     to_glm_vec(s.points[n_ind].first);
    glm::dvec2 norm_end_lap = (glm::distance(end_lap, glm::dvec2(0)) >
                               std::numeric_limits<double>::epsilon())
                                ? glm::normalize(end_lap)
                                : glm::normalize(glm::dvec2(-tan.y, tan.x));
    ...
    draw_log.record_to(std::unique_ptr<DrawSegment>(new DrawSegment(n_pos, n_pos
    + 5. * n_lap, nvgRGBA(150, 50, 12, 255))), FilterAttribute(s.group_ind,
    s.stroke_ind), 0); draw_log.record_to(std::unique_ptr<DrawSegment>(new
    DrawSegment(end_pos, end_pos + 5. * end_lap, nvgRGBA(150, 50, 12, 255))),
    FilterAttribute(s.group_ind, s.stroke_ind), 0);
    }*/
  };
  handle_end(s, 1, 1);
  ...
  // Smooth
  {
    ...
    if (s.totalLen() < input_median_length * 0.2) {
      ...
      size_t right_start =
        std::max(overall_start, s.points.size() - 1 - left_end);
...
Capture
StrokeCut::cut_cornucopia(Capture const &capture,
                          std::map<size_t, size_t> &cutindex_to_index,
                          std::map<size_t, std::vector<size_t>> &c_record,
                          bool output_orig) {
  int max_id = 0;
  ...
  auto is_too_short = [](Sketch const &orig_s, Sketch const &s) -> bool {
    ...
    double short_threshold =
      std::min(short_seg_len, orig_s.totalLen() * short_ratio);
    if (seg_len < short_threshold || s.points.size() == 2) {
#ifdef DEBUG_PRINTOUT
      std::cout << "\t Cut: " << seg_len << " from " << orig_s.totalLen()
                << std::endl;
#endif
    }
  ...
  auto cut_stroke = [&](Sketch const &s,
                        bool &is_closed) -> std::vector<Sketch> {
    std::vector<Sketch> cut;

    {
      ...
      params.set(Parameters::MAX_RESCALE,
                 std::numeric_limits<double>::infinity());
      params.set(
        Parameters::CLOSEDNESS_THRESHOLD,
        std::numeric_limits<double>::epsilon()); // only close the curve if it
                                                 // is closed
      // params.set(Parameters::SHORTNESS_COST, 5);
      // params.setAlgorithm(AlgorithmStage::PRIMITIVE_FITTING, 1);
      params.setAlgorithm(AlgorithmStage::RESAMPLING,
                          2); // Use my version of default resampler which use
                              // MAX_SAMPLING_INTERVAL in the correct scaled way
      ...
      fitter.run(); // to see why this prints debugging output, look at
                    // DebuggingTestImpl in Test.cpp
      ...
      std::vector<int>
        _continuities; // continuity[i] is between curves i and i + 1
      std::vector<FitPrimitive> _primitives(
        fitter.output<PRIMITIVE_FITTING>()->primitives);
      ...
      VectorC<CurvePrimitivePtr> _curves = VectorC<CurvePrimitivePtr>(
        (int)_primIdcs.size(), is_closed ? CIRCULAR : NOT_CIRCULAR);
      VectorC<std::pair<int, int>> _curveRanges =
        VectorC<std::pair<int, int>>((int)_primIdcs.size(), _curves.circular());

      for (int i = 0; i < (int)_primIdcs.size(); ++i) {
        _curveRanges[i] = std::make_pair(_primitives[_primIdcs[i]].startIdx,
                                         _primitives[_primIdcs[i]].endIdx);
      ...
      for (int i = 0; i < (int)_continuities.size(); ++i) {
        ...
        // trim
        Eigen::Vector2d trimPt =
          0.5 * (_curves[i]->endPos() + _curves[i + 1]->startPos());
        ...
        if (_curves.circular() || !_primitives[_primIdcs[i + 1]].isFixed())
          _curves[i + 1]->trim(_curves[i + 1]->project(trimPt),
                               _curves[i + 1]->length());

        // the ranges over which error is computed should not overlap
        if (_continuities[i] == 1)
          std::swap(_curveRanges[i].second,
                    _curveRanges[i + 1].first); // they overlap by 1 originally
        if (_continuities[i] == 2) // they overlap by 2
        {
          _curveRanges[i + 1].first =
            (_curveRanges[i + 1].first + 2) % sampledPts;
          _curveRanges[i].second =
            (_curveRanges[i].second + sampledPts - 2) % sampledPts;
        }
      }

      VectorC<Eigen::Vector2d> _pts(fitter.output<RESAMPLING>()->output->pts());
      auto add_to_stroke = [&](int from, int to, int curv_ind,
                               Sketch &cur_stroke) {
        if (from < 0 || to >= (int)_pts.size())
          return;

        for (VectorC<Eigen::Vector2d>::Circulator circ = _pts.circulator(from);;
             ++circ) {
          int idx = circ.index();
          bool last = (idx == to);
          ...
          cur_stroke.points.emplace_back(
            SketchPoint(pt.x(), pt.y(), tangent.x(), tangent.y(),
                        _curves[curv_ind]->curvature(s)),
            0);
          if (last)
            break;
        }
      ...
      auto find_max_curv = [&](int from, int to, int curv_ind,
                               int &max_ind) -> double {
        double max_curv = -1;
        for (VectorC<Eigen::Vector2d>::Circulator circ = _pts.circulator(from);;
             ++circ) {
      ...
      auto add_to_stroke_prim = [&](CurvePrimitiveConstPtr const primitive,
                                    double _s0, double _s1,
                                    Sketch &cur_stroke) {
        const Eigen::Vector2d &pt0 = primitive->pos(_s0);
        Eigen::Vector2d tangent = primitive->der2(_s0);
        tangent.normalize();
        cur_stroke.points.emplace_back(SketchPoint(pt0.x(), pt0.y(),
                                                   tangent.x(), tangent.y(),
                                                   primitive->curvature(_s0)),
                                       0);

        for (double _s = _s0 + s.sampling_interval;
             _s + s.sampling_interval < _s1;
             _s += s.sampling_interval) // skip pixels
        {
          const Eigen::Vector2d &pt = primitive->pos(_s);
          Eigen::Vector2d tangent = primitive->der2(_s);
          tangent.normalize();

          cur_stroke.points.emplace_back(SketchPoint(pt.x(), pt.y(),
                                                     tangent.x(), tangent.y(),
                                                     primitive->curvature(_s)),
                                         0);
        }
        ...
        tangent.normalize();
        cur_stroke.points.emplace_back(SketchPoint(pt1.x(), pt1.y(),
                                                   tangent.x(), tangent.y(),
                                                   primitive->curvature(_s1)),
                                       0);

        cur_stroke.sampling_interval = s.sampling_interval;
      };

      auto find_max_curv_prim = [&](CurvePrimitiveConstPtr const primitive,
                                    double _s0, double _s1,
                                    double &max_s) -> double {
        double max_curv = -1;

        for (double _s = _s0; _s < _s1;
             _s += s.sampling_interval) // skip pixels
        {
      ...
      auto add_break_prim = [&](CurvePrimitiveConstPtr const primitive,
                                double _s0, double _s1, Sketch &cur_stroke) {
        double max_s;
      ...
      if (!output_orig) {
        ...
        if (prim_output->primitives().size() > 0) {
          ...
          add_break_prim(prim_output->primitives()[0], 0,
                         prim_output->primitives()[0]->length(), cut.back());
          ...
          for (int i = 0; i + 1 < (int)prim_output->primitives().size(); ++i) {
            ...
            add_break_prim(prim_output->primitives()[i + 1], 0,
                           prim_output->primitives()[i + 1]->length(),
                           cut.back());
          }
        }
      } else {
        if (_curveRanges.size() > 0) {
          ...
          cut.back().group_ind = s.group_ind;
          cut.back().stroke_ind = s.stroke_ind;
          add_break(_curveRanges[0].first, _curveRanges[0].second, 0,
                    cut.back());
          for (int i = 0; i + 1 < (int)_curveRanges.size(); ++i) {
            ...
            add_break(_curveRanges[i + 1].first, _curveRanges[i + 1].second,
                      i + 1, cut.back());
          }
        }
      }
    ...
    // Find if there's any pair of curvature cut points that are too close to
    // each other. If yes, split it to its neighbors. Note that this kind of
    // segments can only in the middle of an input stroke, since the start and
    // end of the entire stroke are not curvature cut points.
    {
      for (size_t i = 1; i + 1 < dezero.size(); i++) {
        ...
        if (/*dezero[i].points.front().second == 2 &&
          ...
          if (num_p > 2) {
            dezero[prev_ind].points.insert(
              dezero[prev_ind].points.end(), dezero[i].points.begin() + 1,
              dezero[i].points.begin() + (num_p >> 1));
            dezero[i + 1].points.insert(dezero[i + 1].points.begin(),
                                        dezero[i].points.begin() + (num_p >> 1),
                                        dezero[i].points.end() - 1);
          } else {
          ...
          dezero[prev_ind].points.back().second =
            2; // dezero[i].points.front().second;
          dezero[i + 1].points.front().second =
            2; // dezero[i].points.back().second;
    ...
    // Find if there's any curvature cut point which is significantly larger
    // than its neighbors.
    {
      auto compute_median_curvature = [](std::vector<double> &curv) -> double {
        std::nth_element(curv.begin(), curv.begin() + curv.size() / 2,
                         curv.end());
        return curv[curv.size() / 2];
      };
      auto compute_median_absolute_deviation =
        [](std::vector<double> const &curv, double median) -> double {
        std::vector<double> abs_dev = curv;
        for (double &v : abs_dev)
          v = std::abs(v - median);
      ...
      auto compute_standard_deviation = [&](std::vector<double> const &curv,
                                            double mean) -> double {
      ...
      for (size_t i = 0; i + 1 < declose.size(); i++) {
        if (!declose[i].points.empty() &&
          ...
          double max_curv =
            *std::max_element(local_curv.begin(), local_curv.end());
          ...
          if (ratio < max_curvature_ratio) {
            ...
            declose[i + 1].points.insert(declose[i + 1].points.begin(),
                                         declose[i].points.begin(),
                                         declose[i].points.end());
    ...
    if (is_closed &&
        (cut.front().points.front().first - cut.back().points.back().first)
  ...
  for (size_t i = 0; i < capture.sketchedPolylines.size(); i++) {
    ...
    std::vector<Sketch> cut_strokes =
      cut_stroke(capture.sketchedPolylines[i], is_closed);

#ifdef DEBUG_PRINTOUT
    std::cout << "\t" << capture.sketchedPolylines[i].stroke_ind << ": "
              << is_closed << std::endl;
#endif
    ...
    if (cut_strokes.size() == 0)
      continue;

    if (!is_closed) {
      to_keep.front() =
        !is_too_short(capture.sketchedPolylines[i], cut_strokes.front());
      to_keep.back() =
        !is_too_short(capture.sketchedPolylines[i], cut_strokes.back());
    }
  ...
  for (size_t i = 0; i < cut_capture.sketchedPolylines.size(); i++) {
    ...
    for (size_t j = 0; j + 1 < cut_capture.sketchedPolylines[i].points.size();
         j++) {
      cut_capture.sketchedPolylines[i].sampling_interval +=
        (cut_capture.sketchedPolylines[i].points[j + 1].first -
         cut_capture.sketchedPolylines[i].points[j].first)
          .Length();
    }
    cut_capture.sketchedPolylines[i].sampling_interval =
      (cut_capture.sketchedPolylines[i].points.size() >= 2)
        ? cut_capture.sketchedPolylines[i].sampling_interval /
            cut_capture.sketchedPolylines[i].points.size()
        : 0;
    cut_capture.sketchedPolylines[i].reparameterize(
      cut_capture.sketchedPolylines[i].sampling_interval);
...
void StrokeCut::prepare_cornucopia(Capture &capture) const {
  ...
  for (size_t i = 0; i < capture.sketchedPolylines.size(); i++) {
    double reparam_step_length =
      5.; // std::max(5., capture.sketchedPolylines[i].sampling_interval);
    double step_length =
      (capture.getSketchedPolyline(i).totalLen() / min_num_sample_per_stroke);
    step_length = std::min(step_length, reparam_step_length);
    step_length =
      std::max(step_length, capture.sketchedPolylines[i].sampling_interval);
    // capture.getSketchedPolyline(i).reparameterize(step_length);

    step_length =
      std::max(step_length, capture.getSketchedPolyline(i).totalLen() /
                              max_num_sample_per_stroke);

    size_t num_steps = capture.getSketchedPolyline(i).totalLen() / step_length;

    capture.getSketchedPolyline(i).sampling_interval =
      step_length; // temporarily set this value for cut_cornucopia func
  }
}
