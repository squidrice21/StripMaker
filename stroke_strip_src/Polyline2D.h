...
public: // constructor
  ...
  Point2D(double p_x, double p_y)
    ...
    , tangent_y(0)
    , curvature(0) {}
  Point2D(double p_x, double p_y, double t_x, double t_y)
    : x(p_x)
    , y(p_y)
    , tangent_x(t_x)
    , tangent_y(t_y)
    , curvature(0) {}
  Point2D(double p_x, double p_y, double t_x, double t_y, double curv)
    : x(p_x)
    , y(p_y)
    , tangent_x(t_x)
    , tangent_y(t_y)
...
public: // operations
  ...
  void from_string4svg(double p_x, double p_y) {
    x = p_x;
    y = p_y;
  }

  void from_fits(glm::dvec2 xy) {
    x = xy.x;
    y = xy.y;
  }
};
...
public: // data
  int stroke_ind;
  int group_ind;
  int correct_group_ind;
  std::vector<std::pair<Point2D, std::int64_t>> points;
  double width;
  double sampling_interval;
  std::vector<std::pair<int, bool>> potential_groups;
...
public:
  ...
  // attributes
  // calculate total length of the polyline as the sum of each line segment's
  // length
  ...
  void reparameterize(double sample_step_length, size_t min_samples = 3) {
    ...
    size_t num_samples =
      std::max(min_samples, (size_t)ceil(total / sample_step_length));
    ...
    for (size_t i = 0; i < segLen.size(); i++) {
      ...
      while (acc >= j * sample_step_length) {
        ...
        Point2D reparam_p(
          (1 - local_t) * points[i].first.x + local_t * points[i + 1].first.x,
          (1 - local_t) * points[i].first.y + local_t * points[i + 1].first.y);
        reparam_p.tangent_x = (1 - local_t) * points[i].first.tangent_x +
                              local_t * points[i + 1].first.tangent_x;
        reparam_p.tangent_y = (1 - local_t) * points[i].first.tangent_y +
                              local_t * points[i + 1].first.tangent_y;
        double length = std::sqrt(reparam_p.tangent_x * reparam_p.tangent_x +
                                  reparam_p.tangent_y * reparam_p.tangent_y);
        if (length > std::numeric_limits<double>::epsilon()) {
          reparam_p.tangent_x /= length;
          reparam_p.tangent_y /= length;
        }

        long double reparam_t =
          (1 - local_t) * points[i].second + local_t * points[i + 1].second;
    ...
    if ((points[points.size() - 1].first - reparam_points.back().first)
          .Length() > 1.) {
  ...
  void reindex() {
    ...
    for (size_t i = 1; i < dedup_points.size(); i++) {
      if ((dedup_points[i].first - points.back().first).Length() >
  ...
  double get_t(size_t ind, double seg_t) const {
    assert(ind < points.size());
    ...
    double ind_len = seg[ind];
    if (ind + 1 < points.size()) {
      ind_len = seg[ind] + (seg[ind + 1] - seg[ind]) * seg_t;
    }

    return ind_len / total;
  }
  ...
  bool is_roughly_circular() const {
    ...
    if (points.empty())
      return false;
    if ((points.front().first - points.back().first).Length() <
        std::numeric_limits<double>::epsilon())
      return true;
    ...
    for (size_t i = 1; i + 1 < points.size(); i++) {
      ...
      turning +=
        sign * std::acos(std::max(-1., std::min(glm::dot(t0, t1), 1.)));
    ...
    bool dist_check =
      glm::distance(near_round_p, to_glm_vec(points.at(0).first)) < 20;

    // std::cout << "\t Turn: " << turning/M_PI * 180 << "; " <<
    // glm::distance(near_round_p, to_glm_vec(points.at(0).first)) << std::endl;
  ...
  void laplacian_smooth(size_t center_ind, double t_range) {
    ...
    for (int i = 0; i < smooth_points.size(); i++) {
      if ((std::abs(seg[i] - seg[center_ind]) < t_range ||
           std::abs(i - (int)center_ind) == 1) &&
        ...
        glm::dvec2 smoothed = (wij * to_glm_vec(points[i - 1].first) +
                               wik * to_glm_vec(points[i + 1].first)) /
                              (wij + wik);
  ...
  void laplacian_smooth(size_t center_ind, size_t ind_range) {
    ...
    std::vector<std::pair<Point2D, std::int64_t>> smooth_points = points;
    for (int i = 0; i < smooth_points.size(); i++) {
      if ((std::abs(i - (int)center_ind) <= ind_range) && i != 0 &&
          i + 1 != smooth_points.size()) {
        double wij = 1. / (points[i].first - points[i - 1].first).Length(); // 1
        double wik = 1. / (points[i].first - points[i + 1].first).Length(); // 1
  ...
  void laplacian_move(size_t center_ind, Point2D center_new, size_t ind_range,
                      size_t num_itr = 1) {
    auto to_glm_vec = [](Point2D p) -> glm::dvec2 {
      return glm::dvec2(p.x, p.y);
    };
    ...
    auto compute_laplacians = [&](std::vector<Laplacian> &laplacians) {
      for (size_t i = 1; i + 1 < points.size(); i++) {
        glm::dvec2 neighbor_vec =
          to_glm_vec(points[i + 1].first - points[i - 1].first);
        glm::dvec2 neighbor_dir = glm::normalize(neighbor_vec);
        glm::dvec2 lap =
          to_glm_vec(points[i].first) -
          0.5 * to_glm_vec(points[i + 1].first + points[i - 1].first);
        ...
        double dot_prod =
          std::max(-1., std::min(1., glm::dot(neighbor_dir, lap_dir)));
    ...
    for (size_t k = 0; k < num_itr; k++) {
      ...
      for (int i = 0; i < smooth_points.size(); i++) {
        if (i != 0 && i + 1 != smooth_points.size()) {
          glm::dvec2 avg_neighbor =
            0.5 * to_glm_vec(points[i + 1].first + points[i - 1].first);
          glm::dvec2 neighbor_vec =
            to_glm_vec(points[i + 1].first - points[i - 1].first);
          glm::dvec2 neighbor_dir = glm::normalize(neighbor_vec);
          glm::dmat2x2 rotation(
            std::cos(laplacians[i].angle), -std::sin(laplacians[i].angle),
            std::sin(laplacians[i].angle), std::cos(laplacians[i].angle));
  ...
  Point2D get_endpoint_inward_tangent(size_t end_ind) const {
    ...
    if (end_ind == 0) {
      ...
      tangent =
        points[points.size() - 2].first - points[points.size() - 1].first;
      tangent.Normalize();
    }
  ...
  std::string to_string() const {
    std::string poly_str = "{\n";
    poly_str += "\t#" + std::to_string(stroke_ind) + "\t" +
                std::to_string(group_ind) + "\n";
  ...
  void from_string(std::string poly_str, int ind) {
    ...
    for (size_t i = 0; i < strs.size(); i++) {
      ...
      if (strs[i].find("#") != std::string::npos) {
        ...
        std::vector<std::string> strs_digit =
          SketchUI::split_str(strs[i], "\t");
        stroke_ind = std::stoi(strs_digit[0]);
        group_ind = std::stoi(strs_digit[1]);
        correct_group_ind = std::stoi(strs_digit[1]);
        continue;
      }
      ...
      // Deduplicate
      if (points.size() == 0 || (points.back().first - point).SqLength() >
    ...
    sampling_interval =
      (points.size() >= 2) ? sampling_interval / points.size() : 0;
  }

  void from_fits(FittedCurve curve, int num) {
    stroke_ind = group_ind = num;
    for (auto &p : curve.centerline) {
      Point2D point;
      point.from_fits(p);
  ...
  void from_string4svg(std::string poly_str, size_t ind) {
    stroke_ind = group_ind = -1;
    stroke_ind = ind;
    group_ind = ind;

    width = epsilon_small;
    std::string scd_pattern = "\"";
    std::vector<std::string> features =
      SketchUI::split_str(poly_str, scd_pattern);
    for (size_t i = 0; i < features.size() / 2; i++) {
      if (features[2 * i].find("d=") != std::string::npos) {
        std::string space = " ";
        std::vector<std::string> points_vec =
          SketchUI::split_str(features[2 * i + 1], space);
        for (size_t j = 0; j < points_vec.size() / 3; j++) {
          Point2D point;
          point.from_string4svg(std::stod(points_vec[3 * j + 1]),
                                std::stod(points_vec[3 * j + 2]));
          uint64_t time = 0;
          if (points.size() == 0 || (points.back().first - point).SqLength() >
                                      std::numeric_limits<float>::epsilon()) {
            Point2D prev;
      ...
      if (features[2 * i].find("stroke-width=") != std::string::npos) {
        width = std::stoi(features[2 * i + 1]);
      }
    }
  }
  ...
  std::string to_svg_string() const {
    ...
    for (size_t i = 0; i < points.size(); i++) {
      svg_str += (i == 0) ? "M " : "L ";
      svg_str += std::to_string(points[i].first.x) + " " +
                 std::to_string(points[i].first.y);
      if (i + 1 < points.size())
        svg_str += " ";
    }

    svg_str += "\" fill=\"none\" stroke=\"black\" stroke-width=\"" +
