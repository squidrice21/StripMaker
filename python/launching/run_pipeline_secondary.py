...
"""
Run data generation, training and cross validation for the global classifier.
"""
...
from learning.feature_set import load_secondary_feature_set


def generate_data(path, sample_exe, feature_exe, fea_yml):
    data_list = []
    ...
    if not os.path.exists(output_file + '_sec_filtered.csv'):
        subprocess.run(cmd)

    # Compute features based on the samples
    sample_input_files = [
        output_file + '_sec_pos_cluster.csv',
        output_file + '_sec_neg_cluster.csv',
    ]
    ...
    for sample_f in sample_input_files:
        ...
        cmd = [feature_exe, '-i', path, '-e', sample_f, '-o', output_file,
               ...
               '-c', cache_folder]
        cmd += ['--uncut']
        # Not necessary if we use the first round feature computation
        if fea_yml:
            cmd += ['--fea', fea_yml]
    ...
    if len([f for f in feature_output_files if os.path.exists(f)]) == 0:
        return data_list
    ...
    output_file = path.replace('.scap', '') + '/features_sec_full.csv'
...
def train_data(data_list, model_folder,
               ...
               random_state=42):
    # Learn
    model_file = model_folder + '/model_sec.sav'
    if model_save:
...
def main():
    ...
    if args.fea:
        load_secondary_feature_set(args.fea)
    from learning.feature_set import secondary_feature_set

    # Generate data and learn
    if '.scap' in args.input and args.exe:
        sample_exe, feature_exe = args.exe.split(',')
        data_list = generate_data(
            args.input, sample_exe, feature_exe, args.fea)
        ...
        if not args.cv:
            model_file = train_data(
                ...
                model_save=args.model, feature_set=secondary_feature_set,
